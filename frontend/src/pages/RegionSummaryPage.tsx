...
import { RadioTower, AlertTriangle, AlertOctagon, WifiOff, TrendingUp, TrendingDown, CheckCircle2 } from 'lucide-react';
...
const REGIONS: Region[] = [
  {
    ...
    criticalAlarms: 2,
    stations: [
      { id: 'BSC-001', name: 'Levent-K1', type: 'NR_5G', status: 'CRITICAL', alarmCount: 2 },
      { id: 'BSC-002', name: 'Kadıköy-M3', type: 'LTE', status: 'WARNING', alarmCount: 1 },
      { id: 'BSC-003', name: 'Taksim-A2', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-004', name: 'Beşiktaş-B1', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-005', name: 'Üsküdar-U3', type: 'LTE', status: 'OFFLINE', alarmCount: 2 },
    ],
  },
  {
    name: 'Ege',
    ...
    criticalAlarms: 0,
    stations: [
      { id: 'BSC-006', name: 'Konak-K1', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-007', name: 'Bornova-B2', type: 'LTE', status: 'WARNING', alarmCount: 1 },
      { id: 'BSC-008', name: 'Karşıyaka-K3', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-009', name: 'Buca-B4', type: 'LTE', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-010', name: 'Alsancak-A1', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
    ],
  },
  {
    name: 'İç Anadolu',
    ...
    criticalAlarms: 0,
    stations: [
      { id: 'BSC-011', name: 'Kızılay-K1', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-012', name: 'Çankaya-C2', type: 'NR_5G', status: 'ACTIVE', alarmCount: 0 },
      { id: 'BSC-013', name: 'Etlik-E3', type: 'LTE', status: 'WARNING', alarmCount: 1 },
      { id: 'BSC-014', name: 'Keçiören-K4', type: 'LTE', status: 'WARNING', alarmCount: 1 },
      { id: 'BSC-015', name: 'Mamak-M5', type: 'LTE', status: 'ACTIVE', alarmCount: 0 },
    ],
  },
];
...
export default function RegionSummaryPage() {
  const totalStations = REGIONS.reduce((acc, r) => acc + r.stations.length, 0);
    ...
    <div className="flex flex-col space-y-6 h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {REGIONS.map((region) => {
          ...
          region.stations.forEach((s) => byStatus[s.status]++);
            ...
            <div key={region.name} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-5 border-b border-slate-100">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{region.city}</p>
                  </div>
                  <span
                    className={cn(
                      'inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full',
                      region.trend >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'
                    )}
                  >
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-slate-600">
                  <span>{region.stations.length} istasyon</span>
              ...
              <div className="divide-y divide-slate-50">
                {region.stations.map((station) => {
                  const SIcon = STATUS_ICON[station.status];
                  return (
                    <Link
                      key={station.id}
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 font-mono">{station.id} · {station.type}</p>
                        </div>
                      </div>
